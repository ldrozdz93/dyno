...
private:
  ...
  poly construct_poly(T&& t)
  {
    ...
    else if constexpr( detail::is_a_make_inplace<RawT> ) {
  ...
  template <typename OtherVTable, typename OtherStorage, typename RawT = std::decay_t<T>, typename TagT, /*TODO: find a better way to overload constructor*/
            typename = std::enable_if_t<not detail::is_a_make_inplace<std::decay_t<OtherVTable>>> >
...
public:
  ...
  poly(T&& t, ConceptMap map)
    : vtable_{[&]{
        if constexpr( detail::is_a_make_inplace<RawT> )
  ...
  template <typename T,
            typename ConceptMap,
            typename... Args >
  poly(detail::make_inplace_t<T>, ConceptMap&& map, Args&&... args)
    : vtable_{dyno::complete_concept_map<ActualConcept, typename detail::make_inplace_t<T>::type>(map)}
    , storage_{detail::make_inplace<T>, std::forward<Args>(args)...}
  { }
